...
class GeneExpression:
    ...
    def __init__(self, datasetname='', headerlist=[], genedict={}):
        ...
        The class will store gene names and associated profiles (in which
        values correspond to "samples").

        It also stores headers (which names correspond to samples, i.e.
        experiments).

        :param datasetname: name of data set (if any)
        :param genes: a list of sample names (headerlist; must agree with the
                      number of values in each gene profile)
        :parm genedict: a gene name dictionary where values contain the
                        expression profile (genedict; profile is an iterable
    ...
    def _createMatrix(self, genedict):
        """
        Internal method for constructing a numpy matrix from a gene-profile
        dictionary.
        """
    ...
    def getHeaders(self, indices = None):
        ...
        If indices is None (default), all headers are returned.
        ...
        If indices is a single integer, the header for the corresponding entry
        is returned, e.g.

        >>> g.getHeaders(1)
        'Sample2'

        If indices is an iterable of integers (multiple indices), the list of
        corresponding headers is returned, e.g.
    ...
    def getGenes(self, names = None):
        ...
        If names is a single string, the profile for the corresponding entry
        is returned, e.g.

        >>> g.getGenes('G2')
        array([ 4.1, -0.9])

        If names is an iterable of strings (multiple gene names), a dictionary
        with gene name as key and profile as value is returned.
    ...
    def __getitem__(self, ndx):
        """
        Retrieve a specified sample (or a "slice" of samples) for all genes
        ...
        Note that the order of rows/genes is NOT necessarily the same as that
    ...
    def getSamples(self, samples):
        """
        Construct a gene dictionary including only samples in specified indices
    ...
    def sort(self, sample, descending=True):
        """
        Get a list of gene names, sorted by order of value in specified sample
    ...
    def addSamples(self, headerlist, genedict):
        ...
        Only values for genes in the current set will be added (others are
        ignored).
    ...
    def getZScore(self, index):
        ...
        index can be a list of indices (for which the z-score is computed
        independently).
...
def readGEOFile(filename, id_column=0):
    ...
    id_column indicates what field of each row that should be taken addSamples
    gene identifier.
    """
    fh = open(filename, "rU")
    manylines = fh.read()
